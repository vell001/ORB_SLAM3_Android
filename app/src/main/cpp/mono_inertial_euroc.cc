...
int main(int argc, char *argv[])
{
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            std::cerr << "image: " << vstrImageFilenames[seq][ni] << std::endl;
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            vstrImages.push_back(strImagePath + "/" + data.value("name",""));

            vTimeStamps.push_back(data.value("ts",0.0));
...
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    while(!fImu.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            vTimeStamps.push_back(data.value("ts",0.0));
            vAcc.push_back(cv::Point3f(data.value("ax",0.0),data.value("ay",0.0),data.value("az",0.0)));
            vGyro.push_back(cv::Point3f(data.value("gx",0.0),data.value("gy",0.0),data.value("gz",0.0)));
        }
    }
}
